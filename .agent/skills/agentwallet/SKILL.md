...
### EVM Transfer (ETH or USDC)
```bash
curl -X POST https://agentwallet.mcpay.tech/api/wallets/YOUR_USERNAME/actions/transfer \
  -H "Authorization: Bearer FUND_API_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"to":"0x...","amount":"1000","asset":"usdc","chainId":8453}'
```

| Field | Type | Description |
|-------|------|-------------|
| `to` | string | Recipient address |
| `amount` | string | Amount in smallest units (wei for ETH, 6 decimals for USDC) |
| `asset` | string | `"eth"` or `"usdc"` (default: eth) |
| `chainId` | number | Chain ID (8453 for Base, see supported chains below) |
